...
$id_simpanan = $_GET['id'] ?? 0;

// Ambil data simpanan yang akan diedit
$query_simpanan = "SELECT * FROM simpanan WHERE id_simpanan = ?";
$stmt_simpanan = mysqli_prepare($conn, $query_simpanan);
mysqli_stmt_bind_param($stmt_simpanan, "i", $id_simpanan);
mysqli_stmt_execute($stmt_simpanan);
$result_simpanan = mysqli_stmt_get_result($stmt_simpanan);
$simpanan = mysqli_fetch_assoc($result_simpanan);

if (!$simpanan) {
    $_SESSION['error_message'] = "Data simpanan tidak ditemukan.";
    ...
    <div class="main-content">
        ...
        <div class="card p-4">
            <form action="proses_edit_simpanan.php" method="POST">
                <input type="hidden" name="id_simpanan" value="<?php echo $simpanan['id_simpanan']; ?>">
                <div class="mb-3">
                    ...
                    <select class="form-select" id="id_anggota" name="id_anggota" required>
                        ...
                        <?php while ($row = mysqli_fetch_assoc($result_anggota)) : ?>
                            <option value="<?php echo $row['id_anggota']; ?>" <?php echo ($row['id_anggota'] == $simpanan['id_anggota'] ? 'selected' : ''); ?>><?php echo htmlspecialchars($row['nama']); ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
                <div class="mb-3">
                    ...
                    <select class="form-select" id="jenis_simpanan" name="jenis_simpanan" required>
                        ...
                        <option value="pokok" <?php echo ($simpanan['jenis_simpanan'] == 'pokok' ? 'selected' : ''); ?>>Pokok</option>
                        <option value="wajib" <?php echo ($simpanan['jenis_simpanan'] == 'wajib' ? 'selected' : ''); ?>>Wajib</option>
                        <option value="sukarela" <?php echo ($simpanan['jenis_simpanan'] == 'sukarela' ? 'selected' : ''); ?>>Sukarela</option>
                    </select>
                </div>
                <div class="mb-3">
                    <label for="jumlah" class="form-label">Jumlah</label>
                    <input type="number" class="form-control" id="jumlah" name="jumlah" value="<?php echo htmlspecialchars($simpanan['jumlah']); ?>" required min="0">
                </div>
                <div class="mb-3">
                    <label for="tanggal" class="form-label">Tanggal</label>
                    <input type="datetime-local" class="form-control" id="tanggal" name="tanggal" value="<?php echo date('Y-m-d\\TH:i', strtotime($simpanan['tanggal'])); ?>" required>
                </div>
                <div class="mb-3">
                    <label for="plan" class="form-label">Plan</label>
                    <select class="form-select" id="plan" name="plan" required>
                        <option value="">Pilih Plan</option>
                        <option value="sekali" <?php echo ($simpanan['plan'] == 'sekali' ? 'selected' : ''); ?>>Sekali</option>
                        <option value="bulanan" <?php echo ($simpanan['plan'] == 'bulanan' ? 'selected' : ''); ?>>Bulanan</option>
                    </select>
                </div>
                <div class="mb-3">
                    ...
                    <select class="form-select" id="status" name="status" required>
                        ...
                        <option value="pending" <?php echo ($simpanan['status'] == 'pending' ? 'selected' : ''); ?>>Pending</option>
                        <option value="verified" <?php echo ($simpanan['status'] == 'verified' ? 'selected' : ''); ?>>Verified</option>
                        <option value="canceled" <?php echo ($simpanan['status'] == 'canceled' ? 'selected' : ''); ?>>Canceled</option>
                    </select>
                </div>
                <div class="mb-3">
                    <label for="fee" class="form-label">Fee</label>
                    <input type="number" class="form-control" id="fee" name="fee" value="<?php echo htmlspecialchars($simpanan['fee']); ?>" min="0">
                </div>
                <button type="submit" class="btn btn-primary rounded-pill px-4"><i class="bi bi-save"></i> Simpan Perubahan</button>
                <a href="simpanan.php" class="btn btn-secondary rounded-pill px-4">Batal</a>
            </form>
        </div>
    </div>
