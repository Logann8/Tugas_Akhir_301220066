...
$id_pinjaman = $_GET['id'] ?? 0;

// Ambil data pinjaman yang akan diedit
$query_pinjaman = "SELECT * FROM pinjaman WHERE id_pinjaman = ?";
$stmt_pinjaman = mysqli_prepare($conn, $query_pinjaman);
mysqli_stmt_bind_param($stmt_pinjaman, "i", $id_pinjaman);
mysqli_stmt_execute($stmt_pinjaman);
$result_pinjaman = mysqli_stmt_get_result($stmt_pinjaman);
$pinjaman = mysqli_fetch_assoc($result_pinjaman);

if (!$pinjaman) {
    $_SESSION['error_message'] = "Data pinjaman tidak ditemukan.";
...
<head>
    ...
    <title>Edit Pinjaman - SIKOPIN</title>
...
<body>
    ...
    <div class="main-content">
        <h4 class="fw-bold mb-4">Edit Pinjaman</h4>
        <div class="card p-4">
            <form action="proses_edit_pinjaman.php" method="POST">
                <input type="hidden" name="id_pinjaman" value="<?php echo $pinjaman['id_pinjaman']; ?>">
                <div class="mb-3">
                    ...
                    <select class="form-select" id="id_anggota" name="id_anggota" required>
                        ...
                        <?php while ($row = mysqli_fetch_assoc($result_anggota)) : ?>
                            <option value="<?php echo $row['id_anggota']; ?>" <?php echo ($row['id_anggota'] == $pinjaman['id_anggota'] ? 'selected' : ''); ?>><?php echo htmlspecialchars($row['nama']); ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
                <div class="mb-3">
                    <label for="jumlah_pinjaman" class="form-label">Jumlah Pinjaman</label>
                    <input type="number" class="form-control" id="jumlah_pinjaman" name="jumlah_pinjaman" value="<?php echo htmlspecialchars($pinjaman['jumlah_pinjaman']); ?>" required min="0">
                </div>
                <div class="mb-3">
                    <label for="tanggal_pinjam" class="form-label">Tanggal Pinjam</label>
                    <input type="date" class="form-control" id="tanggal_pinjam" name="tanggal_pinjam" value="<?php echo date('Y-m-d', strtotime($pinjaman['tanggal_pinjam'])); ?>" required>
                </div>
                <div class="mb-3">
                    <label for="tanggal_kembali" class="form-label">Tanggal Kembali</label>
                    <input type="date" class="form-control" id="tanggal_kembali" name="tanggal_kembali" value="<?php echo date('Y-m-d', strtotime($pinjaman['tanggal_kembali'])); ?>" required>
                </div>
                <div class="mb-3">
                    <label for="bunga" class="form-label">Bunga (%)</label>
                    <input type="number" class="form-control" id="bunga" name="bunga" value="<?php echo htmlspecialchars($pinjaman['bunga']); ?>" min="0" step="0.01">
                </div>
                <div class="mb-3">
                    <label for="status" class="form-label">Status</label>
                    <select class="form-select" id="status" name="status" required>
                        <option value="">Pilih Status</option>
                        <option value="pending" <?php echo ($pinjaman['status'] == 'pending' ? 'selected' : ''); ?>>Pending</option>
                        <option value="disetujui" <?php echo ($pinjaman['status'] == 'disetujui' ? 'selected' : ''); ?>>Disetujui</option>
                        <option value="ditolak" <?php echo ($pinjaman['status'] == 'ditolak' ? 'selected' : ''); ?>>Ditolak</option>
                    </select>
                </div>
